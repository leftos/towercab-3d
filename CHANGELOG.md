...
### Highlights
- TowerCab 3D now runs on macOS (Apple Silicon).
- MSFS aircraft models (FSLTL/AIG) can be converted and used on macOS — copy them from a Windows MSFS install.
...
### Highlights
- Fixed the severe slowdown (1-2 FPS) where the app could render on the CPU instead of the GPU — it now uses Direct3D 11 and runs at full speed again.
- New **Graphics Backend** setting (Settings → Graphics) to choose the GPU rendering API, with a warning if the app falls back to software rendering.
- Performance Monitor (F1) now shows real GPU frame time and a breakdown of where idle time goes.
...
- Modded tower cab interiors now pick up environment lighting so glossy surfaces shade more naturally.

### Changed
- WASD in tower-cab repositioning mode is now camera-relative — keys move the cab relative to where the camera is facing.

### Fixed
- Fog and cloud weather effects no longer paint through modded tower cab walls.
