...
export function filterPilotsByAirport(
  ...
  const departing = pilots.filter((p) => p.flight_plan?.departure?.toUpperCase() === normalizedIcao)
  const arriving = pilots.filter((p) => p.flight_plan?.arrival?.toUpperCase() === normalizedIcao)
