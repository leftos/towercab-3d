...
class AircraftModelServiceClass {
  ...
  private checkMsfsModel(
    ...
    const createExactResult = (cachedPath: string, modelName: string): ModelInfo => ({
    ...
    if (msfsSettings.skipGenericVmrMatches && alternatives.length > 0) {
      ...
      if (alternatives.length < originalCount) {
        log(
          '1. VMR rules',
          `filtered ${originalCount - alternatives.length} generic models (skipGenericVmrMatches=true)`,
        )
      }
    }
    ...
    if (airlineCode) {
      ...
      if (closestAirline) {
        ...
        if (result) {
          log(
            '4. closest airline',
            `MATCH ${result.matchType} ${closestAirline.model.modelName} (dist=${closestAirline.distance.toFixed(3)})`,
          )
          return result
        }
      } else {
    ...
    if (closestAny) {
      ...
      if (result) {
        log(
          '6. closest any',
          `MATCH ${result.matchType} ${closestAny.model.modelName} (dist=${closestAny.distance.toFixed(3)})`,
        )
        return result
      }
    } else {
  ...
  getModelInfo(
    ...
    if (!aircraftType) {
      ...
      if (airlineCode && !isGA) {
        ...
        if (airlineFallback) {
          ...
          const cachedPath = MSFSModelConversionService.getCachedModel(
            airlineFallback.modelName,
            airlineFallback.source,
          )
          if (cachedPath) {
          ...
          if (
            !MSFSModelConversionService.isConverting(airlineFallback.modelName, airlineFallback.source) &&
            ...
            MSFSModelConversionService.convertModel(airlineFallback, aircraftPosition)
              .then(() => {
                ...
                window.dispatchEvent(
                  new CustomEvent('model-conversion-complete', { detail: { modelName: airlineFallback.modelName } }),
                )
              })
              .catch(() => {
                this.modelInfoCache.delete(cacheKey)
    ...
    const logFinal = (step: string, result: ModelInfo) => {
      this.logModelMatch(
        ...
        airlineCode,
        step,
        `${result.matchType} -> ${result.matchedModel ?? result.modelUrl}`,
      )
      this.markLogged(callsign)
    }
