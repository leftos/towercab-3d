...
import { getCameraPositionFromManifest, modService } from '../services/ModService'
import type { Airport } from '../types/airport'
import { getEstimatedTowerHeight } from '../types/airport'
import type { ResolvedView2dPosition, View3dPosition } from '../types/mod'
...
export const useAirportStore = create<AirportStore>()((set, get) => ({
  ...
  searchAirports: (query: string): Airport[] => {
    ...
    return results.sort((a, b) => {
      const aExact = a.icao.toLowerCase() === normalizedQuery || a.iata?.toLowerCase() === normalizedQuery
      const bExact = b.icao.toLowerCase() === normalizedQuery || b.iata?.toLowerCase() === normalizedQuery
