    ...
    for (const pilot of allPilots) {
      ...
      if (matchesCallsign || matchesType || matchesDeparture || matchesArrival) {
        ...
        const distance = cameraPosition
          ? calculateDistanceNM(cameraPosition.latitude, cameraPosition.longitude, pilot.latitude, pilot.longitude)
          : Infinity
  ...
  return (
    <>
      ...
      >
        ...
        <span className="search-btn-text">Flights</span>
          ...
          <div className="global-search-overlay" role="presentation" onClick={() => setIsOpen(false)}>
            ...
            <div className="global-search-modal" role="dialog" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="search-results">
                {query.trim() === '' ? (
                  <div className="search-hint">Start typing to search all {allPilots.length} aircraft on VATSIM</div>
                  ...
                  searchResults.map((result) => (
                    ...
                    >
                      ...
                      <div className="result-details">
                        {(result.departure && result.departure !== 'null') ||
                        (result.arrival && result.arrival !== 'null') ? (
                          <span className="result-route">
                            {result.departure && result.departure !== 'null' ? result.departure : '????'} →{' '}
                            {result.arrival && result.arrival !== 'null' ? result.arrival : '????'}
                          </span>
                        ) : null}
                        <span className="result-info">
                          {cameraPosition && result.distance !== Infinity && <>{result.distance.toFixed(1)}nm • </>}
                          FL
                          {Math.round(result.altitude / 0.3048 / 100)
                            .toString()
                            .padStart(3, '0')}{' '}
